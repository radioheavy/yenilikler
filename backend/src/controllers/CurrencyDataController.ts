...
export class CurrencyDataController {
  ...
  async createCurrencyData(req: Request, res: Response): Promise<void> {
    try {
      const currencyData = await this.currencyDataService.createCurrencyData(
        req.body,
      );
  ...
  async getCurrencyData(req: Request, res: Response): Promise<void> {
    try {
      const currencyData = await this.currencyDataService.getCurrencyData(
      ...
      res
        .status(500)
        .json({ message: "Error retrieving currency data", error });
    }
  }

  async updateCurrencyData(req: Request, res: Response): Promise<void> {
    try {
      const currencyData = await this.currencyDataService.updateCurrencyData(
        Number(req.params.id),
        req.body,
      );
      if (currencyData) {
  ...
  async getInflationAdjustedValues(req: Request, res: Response): Promise<void> {
    try {
      const values =
        await this.currencyDataService.calculateInflationAdjustedValues(
          Number(req.params.id),
        );
      res.json(values);
    } catch (error) {
      res.status(500).json({
        message: "Error calculating inflation adjusted values",
        error,
      });
    }
  }
  ...
  async getCurrencyChangeRates(req: Request, res: Response): Promise<void> {
    try {
      const rates = await this.currencyDataService.calculateCurrencyChangeRates(
        Number(req.params.id),
      );
      res.json(rates);
    } catch (error) {
      res
        .status(500)
        .json({ message: "Error calculating currency change rates", error });
    }
  }
}
