...
export default (webSocketServer: WebSocketServer) => {
  ...
  router.get('/:id/inflation-adjusted', (req, res) =>
    currencyDataController.getInflationAdjustedValues(req, res),
  );
  router.get('/:id/change-rates', (req, res) =>
    currencyDataController.getCurrencyChangeRates(req, res),
  );
