...
export class CurrencyDataService {
  ...
  async calculateInflationAdjustedValues(
    id: number,
  ): Promise<{ TRY: number; USD: number; EUR: number }> {
    const currencyData = await this.getCurrencyData(id);
    const shareDetails = await this.shareDetailsRepository.findOne({
      where: { campaign: { id: currencyData?.campaign.id } },
    });
    if (!currencyData || !shareDetails) {
      throw new Error('Currency data or share details not found');
    }
    const usdChange =
      (currencyData.currentUSDRate - currencyData.startDateUSDRate) / currencyData.startDateUSDRate;
    const eurChange =
      (currencyData.currentEURRate - currencyData.startDateEURRate) / currencyData.startDateEURRate;
    const tryInflation =
      shareDetails.distributionSharePriceTRY +
      (shareDetails.distributionSharePriceTRY * (usdChange + eurChange)) / 2;
    const usdInflation =
      shareDetails.distributionSharePriceUSD + shareDetails.distributionSharePriceUSD * usdChange;
    const eurInflation =
      shareDetails.distributionSharePriceEUR + shareDetails.distributionSharePriceEUR * eurChange;
    return {
      TRY: tryInflation,
      USD: usdInflation,
      EUR: eurInflation,
